...
from kotti.compat import _to_fieldstorage
...
def _to_fieldstorage(fp, filename, mimetype, size, **_kwds):
    """ Build a :class:`~kotti.compat.FieldStorage` instance.

    Deform's :class:`FileUploadWidget` returns a dict, but
    :class:`depot.fields.sqlalchemy.UploadedFileField` likes
    :class:`~kotti.compat.FieldStorage` objects

    This function is deprecated. Use kotti.compat._to_fieldstorage instead.
    """
    from kotti.compat import FieldStorage
    f = FieldStorage(fp=fp, filename=filename, mimetype=mimetype, size=size)
    return f
