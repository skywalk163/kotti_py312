...
def _get_alembic_dir() -> str:
    """Get the path to Kotti's alembic directory."""
    alembic_files = files("kotti") / "alembic"
    # For installed packages, we need to get the actual path
    try:
        from importlib.resources import as_file
        with as_file(alembic_files) as path:
            return str(path)
    except ImportError:
...
KOTTI_SCRIPT_DIR = _get_alembic_dir()
